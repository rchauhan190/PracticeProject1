      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              {[...new Set(data.map((item) => item.latestAndroidVersion))].map(
                (version) => (
            ...
            {/* Sort Button */}
            <Button sx={{borderRadius:"50px",color:"#2b5bc7",backgroundColor:"white"}} variant="contained" onClick={handleSortByName}>
              Sort by Name {sortOrder === "asc" ? "⬆️" : "⬇️"}
            </Button>

          <Link
            href="/add-consumer"
            sx={{ display: "flex", justifyContent: "flex-end" ,textDecoration: "none"}}
            >
            <Button sx={{borderRadius:"50px",color:"#2b5bc7",backgroundColor:"white",}} variant="contained" >
              Add Consumer
            </Button>
          </Link>
            </Box>
              ...
              <TableBody>
                {filteredData.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {
                        ...
                        <Button variant="contained"  sx={{ backgroundColor: "#2b5bc7",borderRadius:"20px" }}>
                          Go to Page
                        </Button>
                      </Link>
                    ...
                    <TableCell>
                    <Button variant="contained"  sx={{ backgroundColor: "#2b5bc7",borderRadius:"20px" }} onClick={() => handleOpenQuickView(row)}>
                      Quick View
                    </Button>
                  </TableCell>
                    <TableCell>
                      <Button
                        ...
                        variant="contained"
                        sx={{ backgroundColor: "#2b5bc7",borderRadius:"20px"  }}
                      >
                        Update
                      </Button>
                    </TableCell>
        ...
        <Dialog open={open} onClose={handleCloseQuickView}>
          ...
          <DialogContent>
            {selectedUser && (
              <Box sx={{ padding: 2 }}>
                <Typography><strong>Name:</strong> {selectedUser.consumerName}</Typography>
                <Typography><strong>ID:</strong> {selectedUser.consumerId}</Typography>
                <Typography><strong> Latest Android Version:</strong> {selectedUser.latestAndroidVersion}</Typography>
                <Typography><strong> Minnimum Android Version:</strong> {selectedUser.minimumAndroidVersion}</Typography>
                <Typography><strong>Latest Ios Version:</strong> {selectedUser.latestIosVersion}</Typography>
                <Typography><strong>Web Url:</strong> {selectedUser.webSdkUrl}</Typography>
                <Typography><strong>Created on:</strong> {selectedUser.createdAt}</Typography>
                <Typography><strong>Updated on:</strong> {selectedUser.updatedAt}</Typography>
              </Box>
            )}
          </DialogContent>
          <DialogActions>
