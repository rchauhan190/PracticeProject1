...
export default function UpdateConsumer() {
  ...
  useEffect(() => {
    ...
    if (!consumerId) {
      console.error("Error: Invalid consumerId received.");
      alert("Invalid Consumer ID. Please try again.");
    ...
    const fetchConsumerData = async () => {
        ...
        setConsumerData(response.data.data || response.data);
  ...
  const handleChange = (event) => {
    setConsumerData({ ...consumerData, [event.target.name]: event.target.value });
  };
